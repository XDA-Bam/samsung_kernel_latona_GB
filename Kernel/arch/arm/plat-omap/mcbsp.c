...
#ifdef SIDLE_WA
/* save smart idle enable/disable status */
static int sidle_status = 0;
struct mutex sidle_mutex;
#endif
...
void omap_mcbsp_start(unsigned int id, int tx, int rx)
{
	...
	mutex_lock(&sidle_mutex);
	sidle_status++;
	mutex_unlock(&sidle_mutex);
#endif
...
static int __devinit omap_mcbsp_probe(struct platform_device *pdev)
{
	...
	if (pdata->dma_op_mode != -EINVAL) {
		...
		printk("Mcbsp%d Max Tx = %d, Max Rx = %d\n", (id + 1), mcbsp->max_tx_thres, mcbsp->max_rx_thres);
	}
