...
void __init tf_allocate_workspace(void)
{
	...
	printk(KERN_INFO "SMC: reserved 2MB at physical address 0x%x\n",
		g_SCXLNXDeviceMonitor.nSDPBkExtStoreAddr);
}
#endif /* SMC_EARLY_INIT */

struct smc_sysfs_entry {
	struct attribute attr;
	ssize_t (*show)(SCXLNX_DEVICE_MONITOR *, char *);
	ssize_t (*store)(SCXLNX_DEVICE_MONITOR *, const char *, size_t);
};

/*
 * sysfs entry showing allocation stats
 */
static ssize_t info_show(SCXLNX_DEVICE_MONITOR *pDev, char *buf)
{
	SCXLNX_DEVICE_STATS *pDeviceStats = &pDev->sDeviceStats;
...
static struct smc_sysfs_entry smc_info_entry = __ATTR_RO(info);

/*
 * sysfs entry enabling/disabling crypto safe mode
 */
static ssize_t crypto_safe_mode_show(SCXLNX_DEVICE_MONITOR *pDev, char *buf)
{
	int crypto_safe_mode_on = test_bit(SCXLNX_DEVICE_FLAG_CRYPTO_SAFE_MODE,
		&pDev->nFlags);

	return snprintf(buf, PAGE_SIZE, "%s\n",
			crypto_safe_mode_on ? "on" : "off");
}
static ssize_t crypto_safe_mode_store(SCXLNX_DEVICE_MONITOR *pDev,
	const char *buf, size_t len)
{
	char *e;
	unsigned long long on = simple_strtoull(buf, &e, 10);

	if (e == buf || (*e && *e != '\n'))
		return -EINVAL;

	if (on) {
		printk(KERN_INFO "SMC: Enabling safe crypto mode\n");
		set_bit(SCXLNX_DEVICE_FLAG_CRYPTO_SAFE_MODE, &pDev->nFlags);
	} else {
		printk(KERN_INFO "SMC: Disabling safe crypto mode\n");
		clear_bit(SCXLNX_DEVICE_FLAG_CRYPTO_SAFE_MODE, &pDev->nFlags);
	}

	return len;
}
static struct smc_sysfs_entry smc_crypto_safe_mode_entry =
	__ATTR(crypto_safe_mode, S_IRUGO | S_IWUSR,
		crypto_safe_mode_show, crypto_safe_mode_store);

static ssize_t smc_attr_show(struct kobject *kobj, struct attribute *attr,
	char *page)
{
	struct smc_sysfs_entry *entry = container_of(attr,
		struct smc_sysfs_entry, attr);
	SCXLNX_DEVICE_MONITOR *pDevice = container_of(kobj,
		SCXLNX_DEVICE_MONITOR, kobj);

	if (!entry->show)
		return -EIO;

	return entry->show(pDevice, page);
}

static ssize_t smc_attr_store(struct kobject *kobj, struct attribute *attr,
	const char *page, size_t length)
{
	...
	return entry->store(pDevice, page, length);
}

static void smc_kobj_release(struct kobject *ko)
{
	SCXLNX_DEVICE_MONITOR *pDevice = container_of(ko,
		SCXLNX_DEVICE_MONITOR, kobj);

	clear_bit(SCXLNX_DEVICE_SYSFS_REGISTERED, &(pDevice->nFlags));
}

static struct attribute *smc_default_attrs[] = {
	&smc_info_entry.attr,
	&smc_crypto_safe_mode_entry.attr,
	NULL,
};
static const struct sysfs_ops smc_sysfs_ops = {
	.show		= smc_attr_show,
	.store		= smc_attr_store,
};
static struct kobj_type smc_ktype = {
	.release	= smc_kobj_release,
	.sysfs_ops	= &smc_sysfs_ops,
	.default_attrs	= smc_default_attrs,
};
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 27)
	nError = kobject_init_and_add(&(pDevice->kobj),
		&smc_ktype, NULL, "%s", SCXLNX_DEVICE_BASE_NAME);
	if (nError != 0) {
		dprintk(KERN_ERR
...
#else
	kobject_set_name(&pDevice->kobj, SCXLNX_DEVICE_BASE_NAME);
	pDevice->kobj.ktype = &smc_ktype;
	nError = kobject_register(&pDevice->kobj);
	if (nError != 0) {
		dprintk(KERN_ERR
			"SCXLNXDeviceRegister: \
			kobject_register failed [%d] !\n",
...
static void SCXLNXDeviceCleanup(void)
{
	...
	if (test_bit(SCXLNX_DEVICE_SYSFS_REGISTERED,
						&(pDevice->nFlags)) != 0) {
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 27)
		kobject_del(&g_SCXLNXDeviceMonitor.kobj);
#else
		kobject_unregister(&g_SCXLNXDeviceMonitor.kobj);
#endif
	}
}
