...
static int yas_acc_core_driver_init(struct yas_acc_private_data *data)
{
    ...
    err = driver->set_position(CONFIG_INPUT_BMA023_POSITION);
    if (err != YAS_NO_ERROR) {
        kfree(driver);
        return err;
    }
...
#define BMA023_RESOLUTION               256
#define MAX_RETRY_COUNT 30

  
static int bma023_fast_calibration(struct acceleration orientation)
{
   int sum_x = 0;    
   int sum_y = 0;     
   int sum_z = 0;    
   ...
   unsigned char range = yas_bma150_read_bits( BMA023_RANGE);    
   unsigned char bandwidth = yas_bma150_read_bits(BMA023_BANDWIDTH);        
   yas_bma150_update_bits(BMA023_RANGE, BMA023_RANGE_2G);    
   yas_bma150_update_bits(BMA023_BANDWIDTH, BMA023_BANDWIDTH_25HZ);

   
   do {        /* read acceleration data */       
   	if (yas_acc_measure(data->driver, &accel)!= 0) {            
		printk(KERN_ERR "I2C block read error: \n");            
		for (i = 0; i < 3; i++) {
			raw[i] = 0;            
		}        
	} else {            
		...
		printk("cur (%d, %d, %d)\n", raw[0], raw[1], raw[2]);        
		sum_x += (raw[0] -orientation.x * BMA023_RESOLUTION);        
		sum_y += (raw[1] -orientation.y * BMA023_RESOLUTION);        
		sum_z += (raw[2] -orientation.z * BMA023_RESOLUTION);    
    } while (retry--); 
   
   printk("sum (%d, %d, %d)\n", sum_x, sum_y, sum_z);        
   data->cal_data.x = sum_x / MAX_RETRY_COUNT;    
   data->cal_data.y = sum_y / MAX_RETRY_COUNT;    
   data->cal_data.z = sum_z / MAX_RETRY_COUNT;     
   
   old_fs = get_fs();    
   set_fs(KERNEL_DS);    
   
   filp_calibrate = filp_open("/efs/calibration_data", O_CREAT|O_TRUNC|O_WRONLY,0666);
   ...
   len = filp_calibrate->f_op->write(filp_calibrate, (char*)&data->cal_data.x, sizeof(int), &filp_calibrate->f_pos);
   ...
   len = filp_calibrate->f_op->write(filp_calibrate, (char*)&data->cal_data.y, sizeof(int), &filp_calibrate->f_pos);   
   len = filp_calibrate->f_op->write(filp_calibrate, (char*)&data->cal_data.z, sizeof(int), &filp_calibrate->f_pos);    
   filp_close(filp_calibrate, current->files);    
   set_fs(old_fs);
   
   exit:    
   	yas_bma150_update_bits(BMA023_RANGE, range);    
	yas_bma150_update_bits(BMA023_BANDWIDTH, bandwidth);    
	return 0;
}

void bma023_init_calibration(void)
{	
	struct file *filp_calibrate = NULL;	
	mm_segment_t old_fs;
	struct yas_acc_private_data *data = yas_acc_get_data();
	data->cal_data.x = data->cal_data.y = data->cal_data.z = 0;    	
	old_fs = get_fs();	set_fs(KERNEL_DS);	
	filp_calibrate = filp_open("/efs/calibration_data", O_RDONLY, 0666);	
	if(IS_ERR(filp_calibrate))	{		
		printk(KERN_ERR "%s: Can't open the file for calibration\n", __func__);		
		return;	
	}	

	filp_calibrate->f_op->read(filp_calibrate, (char*)&data->cal_data.x, sizeof(int), &filp_calibrate->f_pos);		
	filp_calibrate->f_op->read(filp_calibrate, (char*)&data->cal_data.y, sizeof(int), &filp_calibrate->f_pos);		
	filp_calibrate->f_op->read(filp_calibrate, (char*)&data->cal_data.z, sizeof(int), &filp_calibrate->f_pos);		
	printk(KERN_ERR "%s (%u,%u,%u)\n", 
		__func__, data->cal_data.x, data->cal_data.y, data->cal_data.z);	
	filp_close(filp_calibrate, current->files);	
	set_fs(old_fs);
}

static ssize_t bma023_calibration_show(struct device *dev,
                                       struct device_attribute *attr, char *buf)
{
}

static ssize_t bma023_calibration_store(struct device *dev,
                                        struct device_attribute *attr,
                                        const char *buf, size_t count)
{

    struct acceleration orientation = {0, 0, 1};
	
    printk("### bma023_calibration start\n");
	
    bma023_fast_calibration(orientation);
	
    printk("### bma023_calibration end\n");
...
static DEVICE_ATTR(calibration, S_IRUGO|S_IWUSR|S_IWGRP,
                   bma023_calibration_show, bma023_calibration_store);
...
static void yas_acc_work_func(struct work_struct *work)
{
	...
	//printk("\n yas_acc_work_func delay=%d \n",delay);
	
    accel.xyz.v[0] = accel.xyz.v[1] = accel.xyz.v[2] = 0;
    yas_acc_measure(data->driver, &accel);

    accel.xyz.v[0] -= data->cal_data.x;    
    accel.xyz.v[1] -= data->cal_data.y;    
    accel.xyz.v[2] -= data->cal_data.z;
...
#if defined(CONFIG_HAS_EARLYSUSPEND)
static int bma023_early_suspend(struct early_suspend *handler)
{
/*
    struct yas_acc_private_data *data =yas_acc_get_data();
    struct yas_acc_driver *driver = data->driver;
...
#if defined(CONFIG_HAS_EARLYSUSPEND)
static int bma023_early_resume(struct early_suspend *handler)
{
/*    struct yas_acc_private_data *data =yas_acc_get_data();
    struct yas_acc_driver *driver = data->driver;
    int delay;
...
static int bma023_init_proc()
{
    struct yas_acc_private_data *data;
    int err;
     printk(KERN_ERR "bma023_init_proc +\n");
    /* Setup private data */
    ...
    data->client = omap_gpio_i2c_init(OMAP_GPIO_SENSOR_I2C_SDA, OMAP_GPIO_SENSOR_I2C_SCL, 0x38, 200);
/*
    // Setup i2c client
...
*/
    ...
    data->early_suspend.suspend = (void *)bma023_early_suspend;
    data->early_suspend.resume  = (void *)bma023_early_resume;
    register_early_suspend(&data->early_suspend);
#endif	

	bma023_init_calibration();
...
yas_acc_set_enable(data->driver, 1);

//for(i=0;i<10;i++) 
{
	yas_acc_measure(data->driver, &accel);
	printk("\n yas_acc_test X:%d Y:%d Z:%d \n", accel.xyz.v[0], accel.xyz.v[1], accel.xyz.v[2]);
}
#endif
////////////////////for test///////////////////////////
