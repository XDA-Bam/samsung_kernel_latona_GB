...
#    toolbox cat /system/lib/libsqlite_jni.so > /data/cf-root/backup/libsqlite_jni.so
    ...
    toolbox cat /system/build.prop > /data/cf-root/backup/build.prop

# Applying patched build.prop
    toolbox cat /res/misc/build.prop > /system/build.prop
    toolbox chown 0.0 /system/build.prop
    toolbox chown 0.0 /system/build.prop
    toolbox chmod 755 /system/build.prop
    toolbox chmod 755 /system/build.prop
...
# Free some space from /system as it fully packed in stock odexed roms
    if /sbin/ext/busybox [ -f /system/app/SamsungApps.apk ]; 
    then
        toolbox rm /system/app/SamsungApps.apk
    fi;
    if /sbin/ext/busybox [ -f /system/app/SamsungApps.odex ]; 
    then
        toolbox rm /system/app/SamsungApps.odex
    fi;
