...
# Remove the CWM-installed flag
    $TOOLBOX rm /system/CWM/*

# Backup original WiFi modules
if $TOOLBOX [ ! -f /data/CWM/backup/tiwlan_drv.ko ]; then
    $TOOLBOX cat /system/etc/wifi/tiwlan_drv.ko > /data/CWM/backup/tiwlan_drv.ko
fi;
if $TOOLBOX [ ! -f /data/CWM/backup/tiap_drv.ko ]; then
    $TOOLBOX cat /system/etc/wifi/softap/tiap_drv.ko > /data/CWM/backup/tiap_drv.ko
fi;
